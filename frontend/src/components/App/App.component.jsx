...
import RotatingPics from '../Lib/RotatingPics.component';
...
const Header = ({ pics }) => {
    return (
        <>
            <Nav />
            <RotatingPics pics={pics.length < 4 ? 
                                    [Cigars, Lounge, PepBurn, Store] : pics} />
        </>
    )
}
...
const App = () => {
    ...
    useEffect(() => {
        sendToSrvr('api/v1/general/landing_pictures')
            .then(formatted => setPics(formatted));
    }, [])

    return (
        <GlobalStore stateI={initialState}>
            <Routing className={"full-container"} Header={<Header pics={pics["data"] || []} />} routes={routes}>
            </Routing>
        </GlobalStore>
    );
}
